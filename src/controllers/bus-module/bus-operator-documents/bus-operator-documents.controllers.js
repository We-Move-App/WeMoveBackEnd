...
const getDocument = catchAsyncError(async (req, res, next) => {
  ...
  const userDocuments = await BusOperatorDocumentModel.findOne({ userId: _id })
    .populate({
      path: "documentIds",
...
const addDocument = catchAsyncError(async (req, res, next) => {
  ...
  const userDocument = await BusOperatorDocumentModel.findOne({
    userId,
  ...
  const updatedDocs = await Promise.all(
    validDocumentIds.map(async (documentId) => {
      ...
      if (newFile?.path) {
        ...
        // const cloudImage = await uploadImageOnCloudinary(newFile.path);
        const cloudImage = await uploadImageOnAws(
          newFile.path,
...
const deleteDocument = catchAsyncError(async (req, res, next) => {
  ...
  const user = await BusOperatorDocumentModel.findOneAndUpdate(
    { userId: user_id },
