...
const createHotelPolicy = catchAsyncError(async (req, res, next) => {
    ...
    if (!hotelId || !checkInTime || !checkOutTime || !amenities) {
        return next(new ApiError(statusCode.BAD_REQUEST, "Hotel ID, check-in time, check-out time, and amenities are required."));
    }
    if (!req.files?.hotel_license?.length) {
        return next(new ApiError(statusCode.BAD_REQUEST, "Please upload the hotel license document."));
    }
    ...
    if (existingPolicy) {
        return next(new ApiError(statusCode.CONFLICT, "Hotel policy already exists."));
    }
    let parsedAmenities;
    try {
        ...
        return next(new ApiError("Amenities must be a valid JSON array.", statusCode.BAD_REQUEST));
    }
    const hotelimage = req.files.hotel_license[0];
    const uploadResult = await uploadImageOnAws(hotelimage.path, "hotel-documents");
    ...
    if (user.verificationStatus == "submitted") {
        const hotelManager = await HotelManagerModel.findByIdAndUpdate(_id, { verificationStatus: "processing" }, { new: true });

        // console.log("Hotel status:", hotelManager.verificationStatus);
        // return next(new ApiError(statusCode.FORBIDDEN, "User verification status is not processing."));
    }
    await newPolicy.save();

    return res.status(statusCode.CREATED).json(new ApiResponse(statusCode.CREATED, newPolicy, "Hotel policy created successfully."));
});
...
const getHotelPolicy = catchAsyncError(async (req, res, next) => {
    ...
    const hotelPolicy = await HotelPolicyModel.findOne({ hotelId }).select(" hotelId checkInTime checkOutTime amenities uploadDocuments createdAt updatedAt");
    if (!hotelPolicy) {
    ...
    return res.status(statusCode.OK).json(new ApiResponse(statusCode.OK, response, "Hotel policy retrieved successfully.",));
});
...
const updateHotelPolicy = catchAsyncError(async (req, res, next) => {
    ...
    if (req.files?.hotel_license?.length) {
        ...
        if (!uploadResult) {
            return next(new ApiError(statusCode.INTERNAL_SERVER_ERROR, "Failed to upload hotel license document."));
        }
    ...
    return res.status(statusCode.OK).json(new ApiResponse(statusCode.OK, hotelPolicy, "Hotel policy updated successfully.",));
});
...
module.exports = { createHotelPolicy, getHotelPolicy, updateHotelPolicy, deleteHotelPolicy };
