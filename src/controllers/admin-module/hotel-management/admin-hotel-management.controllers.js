...
const getHotelByManagerId = catchAsyncError(async (req, res, next) => {
  ...
  try {
    ...
    // 2) Fetch bank info
    const bankAccount = await HotelManagerBankModel.findOne({ userId: manager._id });
    ...
    const hotelAddress = await HotelAddressModel.findOne({ hotelId: hotel._id }).populate("address");

    // 7) Fetch standard & luxury rooms and their images
    const standardRoom = await Room.findOne({ hotelId: hotel._id, roomType: "standard" });
    const standardRoomImages = standardRoom ? await HotelRoomImagesModel.findOne({ roomId: standardRoom._id }) : null;

    const luxuryRoom = await Room.findOne({ hotelId: hotel._id, roomType: "luxury" });
    const luxuryRoomImages = luxuryRoom ? await HotelRoomImagesModel.findOne({ roomId: luxuryRoom._id }) : null;

    const standardRoomWithImages = standardRoom ? { ...standardRoom.toObject(), images: standardRoomImages?.images || [] } : null;
    const luxuryRoomWithImages = luxuryRoom ? { ...luxuryRoom.toObject(), images: luxuryRoomImages?.images || [] } : null;
    ...
    res.status(200).json(
      new ApiResponse(200, {
        ...
        bankAccount,
        wallet,
        hotel: { ...hotel.toObject(), address: hotelAddress?.address || null, images: hotelImages?.images || [] },
...
const updateHotelManagerFromAdmin = catchAsyncError(async (req, res, next) => {
  ...
  try {
    ...
    if (profileInfo?.email) updateManagerData.email = profileInfo.email.toLowerCase();
    if (profileInfo?.phoneNumber) updateManagerData.phoneNumber = profileInfo.phoneNumber;
    if (profileInfo?.companyName) updateManagerData.companyName = profileInfo.companyName;
    if (profileInfo?.companyAddress) updateManagerData.companyAddress = profileInfo.companyAddress;
    if (profileInfo?.businessLicense) updateManagerData.businessLicense = profileInfo.businessLicense;
...
const updateBankData = {
  ...
  isPrimary: bankInfo.isPrimary !== undefined ? bankInfo.isPrimary : existingBank.isPrimary
};
...
const bankAccount = await HotelManagerBankModel.findOneAndUpdate(
  ...
  { new: true }
    ...
    if (hotelInfo?.businessLicense) updateHotelData.businessLicense = hotelInfo.businessLicense;
    if (hotelInfo?.totalRoom) updateHotelData.totalRoom = hotelInfo.totalRoom;
    if (hotelInfo?.description) updateHotelData.description = hotelInfo.description;
    ...
    if (hotelInfo?.hotelImages?.length) {
      const existingImages = await hotelImagesModel.findOne({ hotelId: hotel._id });
      if (existingImages?.images?.length) deletedFields.hotelImages = existingImages.images;

      const imagesToSave = hotelInfo.hotelImages.map(img => (typeof img === "string" ? { url: img } : img));
      await hotelImagesModel.findOneAndUpdate(
        { hotelId: hotel._id },
    ...
    if (addressInfo) {
      const hotelAddress = await HotelAddressModel.findOne({ hotelId: hotel._id });
      if (hotelAddress) {
        await AddressModel.findByIdAndUpdate(hotelAddress.address, { ...addressInfo, updatedBy: adminId }, { new: true });
      } else {
        const newAddress = await AddressModel.create({ ...addressInfo, createdBy: adminId });
        await HotelAddressModel.create({ hotelId: hotel._id, address: newAddress._id, createdBy: adminId });
      }
    }
    ...
    const updateRoom = async (type, count, price, amenities, images) => {
      ...
      if (amenities) roomUpdate.amenities = amenities.map(a => ({ name: a.name, status: a.status }));
      roomUpdate.updatedBy = adminId;

      const room = await Room.findOneAndUpdate({ hotelId: hotel._id, roomType: type }, roomUpdate, { new: true });
      if (images?.length) {
        const existingRoomImages = await HotelRoomImagesModel.findOne({ roomId: room._id });
        if (existingRoomImages?.images?.length) deletedFields[`${type}RoomImages`] = existingRoomImages.images;

        const newImages = images.map(img => (typeof img === "string" ? { url: img } : img));
        await HotelRoomImagesModel.findOneAndUpdate({ roomId: room._id }, { images: newImages, updatedBy: adminId }, { upsert: true, new: true });
        updatedFields[`${type}RoomImages`] = newImages;
      }
    };

    if (roomInfo) {
      await updateRoom("standard", roomInfo.standardRoomCount, roomInfo.standardRoomPrice, roomInfo.standardAmenities, roomInfo.standardImages);
      await updateRoom("luxury", roomInfo.luxuryRoomCount, roomInfo.luxuryRoomPrice, roomInfo.luxuryAmenities, roomInfo.luxuryImages);
    }

    // 6) Update Policy
    if (policyInfo) {
      const existingPolicy = await HotelPolicyModel.findOne({ hotelId: hotel._id });
      const updatePolicyData = {};
      if (policyInfo?.checkInTime) updatePolicyData.checkInTime = policyInfo.checkInTime;
      if (policyInfo?.checkOutTime) updatePolicyData.checkOutTime = policyInfo.checkOutTime;
      if (policyInfo?.amenities) updatePolicyData.amenities = policyInfo.amenities.map(a => ({ name: a.name, status: a.status }));

      if (policyInfo?.uploadDocuments?.length) {
        if (existingPolicy?.uploadDocuments?.length) deletedFields.policyDocs = existingPolicy.uploadDocuments;
    ...
    const standardRoomWithImages = { ...updatedStandardRoom.toObject(), images: updatedStandardRoomImages?.images || [] };
    const luxuryRoomWithImages = { ...updatedLuxuryRoom.toObject(), images: updatedLuxuryRoomImages?.images || [] };
    ...
    res.status(200).json(
      new ApiResponse(200, {
        ...
        hotel: { ...hotel.toObject(), address: updatedHotelAddress?.address || null, images: updatedHotelImages?.images || [] },
        standardRoom: standardRoomWithImages,
        luxuryRoom: luxuryRoomWithImages,
        policy,
        updatedFields,
