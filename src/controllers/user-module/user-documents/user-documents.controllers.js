...
const getDocument = catchAsyncError(async (req, res, next) => {
  ...
  const userDocuments = await UserDocumentModel.findOne({ userId: _id })
    .populate({
      path: "documentIds",
      select: "documentName file documentType",
    })
    .exec();
...
const addDocument = catchAsyncError(async (req, res, next) => {
  ...
  const userDocument = await UserDocumentModel.findOne({ userId }).populate(
    "documentIds"
  );
...
const deleteDocuments = catchAsyncError(async (req, res, next) => {
  const { _id } = req.user;
  let user_id;
  ...
  const user = await UserDocumentModel.findOneAndUpdate(
    { userId: user_id },
...
const deleteDocumentById = catchAsyncError(async (req, res, next) => {
  const { _id } = req.user;
  const docId = req.params.id;

  if (!mongoose.Types.ObjectId.isValid(docId)) {
    throw new ApiError(statusCode.BAD_REQUEST, "Invalid document ID provided");
  }

  const documentToDelete = await DocumentsModel.findById(docId);
  if (!documentToDelete) {
  ...
  // Delete the document from AWS if it has a public ID
  const publicId = documentToDelete.file?.public_id;
  if (publicId) {
    await deleteImageFromAws(publicId);
  }
