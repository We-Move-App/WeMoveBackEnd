...
const getAutocomplete = async (input, lat = null, lng = null) => {
  ...
  // 🗺️ Build autocomplete URL with both India + Cameroon restriction
  // ✅ Changed here to allow both CM and IN instead of only one
  let url = `${baseUrl}?input=${encodeURIComponent(input)}&key=${apiKey}`;

  // Add nearby bias if coordinates provided
  if (latNum !== null && lngNum !== null) {
    url += `&location=${latNum},${lngNum}&radius=10000`;
  }
